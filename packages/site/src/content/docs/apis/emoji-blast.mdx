...
These values can be configured as follows, with defaults as _(`value`)_ here.
The `acceleration` and `angle` properties of `gravity` are optional:

- `framerate` _(`60`)_: Expected frames per second to adjust position and velocity changes by.
- `gravity`:
  - `acceleration` _(`0.35`)_: How much to increase velocity in the gravity angle's direction each tick.
  - `angle` _(`180`)_: Direction to apply gravity in, in degrees clockwise from straight up (e.g. `180` is straight down).
...
```javascript
import { emojiBlast, defaultPhysics } from "emoji-blast";
...
emojiBlast({
	physics: {
		...
		initialVelocities: {
			y: {
				max: -defaultPhysics.initialVelocities.y.max,
				min: -defaultPhysics.initialVelocities.y.min,
			},
		},
	},
});
